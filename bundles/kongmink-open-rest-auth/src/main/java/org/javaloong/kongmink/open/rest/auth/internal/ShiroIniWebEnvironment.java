...
public class ShiroIniWebEnvironment extends Pac4jIniEnvironment {

    private static final Logger LOG = LoggerFactory.getLogger(ShiroIniWebEnvironment.class);
    ...
    private final String shiroIniFile;

    public ShiroIniWebEnvironment(String shiroIniFile) {
        LOG.info("Initializing the Web Environment using {}", ShiroIniWebEnvironment.class.getName());
    ...
    private Ini createShiroIni() {
        final File f = new File(shiroIniFile);
        if(f.exists()) {
